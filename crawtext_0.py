		...
		try:
			...
			for e in r.json()['d']['results']:
				if url not in self.db.queue.find(url): 
					self.db.queue.insert({"url":e['Url']})
			return True
		except:
			self.db.report.insert({"url": None, "error_code": "404", "type": "Error Fetching content via Bing API. Please check your API key"})
			return False
...
class Crawl():
	...
	def __init__(self, cfg):
		...
		if 'database_name' and cfg['database_name']!='':
			self.db = Database(str(cfg['database_name']))
