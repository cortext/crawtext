...
class Page():
	...
	def retrieve(self):
		...
		try:
			...
			#self.redirect = self.r.history
			return True
		except requests.exceptions.ConnectionError:
			self.req.status_code = 404
			self.req.description = "Connexion Error"
			#self.db.report.insert({"url":url, "status_code":self.req.status_code, "description": "Connexion Error"})
			return False
		except Exception:
			self.req.status_code = 404
			self.req.description = "Invalid url format"
	...
	def extract_content(self):
		...
		try:
			self.soup = BeautifulSoup(self.src).getHTML()
			return True
		except Exception, e:
			self.req.status_code = 404
			self.req.description = "Error while extracting content"
			return False
		#Extractor(html=self.src).getHTML())
	...
	def extract_urls(self):
		...
		for e in self.soup.find_all('a', {'href': True}):
			url = e.attrs['href']
			#clean url ==> relative Vs absolute
