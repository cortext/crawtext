	...
	with open(filename, 'w') as f:
		...
		for k, v in self.task.items():
			if k not in ["action", "status","msg", "date", "creation_date", "_id"]:
				if k == "creation_date":
					v = self.task[k].strftime('%d-%m-%Y %H-%M-%S')
					f.write(str(k)+": "+str(v)+"\n")
				try:
					f.write(str(k)+": "+str(v)+"\n")
				except Exception:
		...
		date_list = [n.strftime('%d-%m-%Y %H-%M-%S') for n in self.task["date"]]
		status_list = list(zip(self.task["action"], self.task["status"],self.task["msg"], date_list))
